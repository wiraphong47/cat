...
<head>
    ...
    <style>
        ...
        .btn-edit, .btn-delete {
            margin-top: 5px; background-color: #ec407a; border: none;
            color: white; padding: 10px 20px; border-radius: 8px;
            font-weight: bold; text-align: center; width: 100%; display: block;
        }
        .btn-edit:hover, .btn-delete:hover { background-color: #d81b60; }
    </style>
</head>
<body>
    ...
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo "<a href='edit_cat.php?id=" . $row['id'] . "' class='btn-edit'>แก้ไข</a>";
                echo "<a href='delete_cat.php?id=" . $row['id'] . "' class='btn-delete'>ลบ</a>";
                echo "</div>";
                echo "</div>";
                echo "</div>";
            }
